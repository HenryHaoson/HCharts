...
public class SingleBarView extends View {
    ...
    private SingleBarListener listener;
    ...
    public SingleBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.HCharts, defStyleAttr, 0);
        int n = array.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = array.getIndex(i);
            if (attr == R.styleable.HCharts_xyColor) {
                lineColor = array.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.HCharts_leftColor) {// 默认颜色设置为黑色
                leftColor = array.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.HCharts_leftColorBottom) {
                lefrColorBottom = array.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.HCharts_selectLeftColor) {// 默认颜色设置为黑色
    ...
    public void setListener(SingleBarListener listener) {
     ...
     * 点击事件监听接口
     */
    public interface SingleBarListener {
        void onSingleBarClicked(int position);
    }
}
